...
import { OCCASIONS_ADHKAR, ADHKAR, EVENING_ADHKAR, QIYAM_ADHKAR, IFTAR_ADHKAR, TRAVEL_ADHKAR, BACKGROUND_SETS, daySetIndexForToday, backgroundFor } from "@/lib/data";
...
const BG_OFFSET = ADHKAR.length + EVENING_ADHKAR.length + QIYAM_ADHKAR.length + IFTAR_ADHKAR.length + TRAVEL_ADHKAR.length;

export default function OccasionsAdhkar({
  ...
  const idx = Math.min(day.occasionsAdhkarIndex, OCCASIONS_ADHKAR.length - 1);
  const item = OCCASIONS_ADHKAR[idx];
  const count = day.occasionsAdhkarRepCounts[item.id] || 0;
  ...
  const advance = useCallback(() => {
    ...
    setTimeout(() => {
      if (idx + 1 >= OCCASIONS_ADHKAR.length) {
        setDay(d => ({ ...d, occasionsAdhkarCompleted: true }));
        onDone();
      } else {
        setDay(d => ({ ...d, occasionsAdhkarIndex: idx + 1 }));
      }
    }, 380);
  }, [idx, setDay, onDone]);
  ...
  const increment = useCallback(() => {
    ...
    setDay(d => ({ ...d, occasionsAdhkarRepCounts: { ...d.occasionsAdhkarRepCounts, [item.id]: newCount } }));
  ...
  const pct = Math.round((idx / OCCASIONS_ADHKAR.length) * 100 + (count / item.repetitions) * (100 / OCCASIONS_ADHKAR.length));
  ...
  return (
    <div style={{ minHeight: "100vh", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <TopBar
          title={`Occasion · ${idx + 1} / ${OCCASIONS_ADHKAR.length}`}
          onBack={onExit}
          chip
          right={idx > 0 ? (
