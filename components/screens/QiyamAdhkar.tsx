...
import { QIYAM_ADHKAR, ADHKAR, EVENING_ADHKAR, BACKGROUND_SETS, daySetIndexForToday, backgroundFor } from "@/lib/data";
...
const BG_OFFSET = ADHKAR.length + EVENING_ADHKAR.length;

export default function QiyamAdhkar({
  ...
  const idx = Math.min(day.qiyamAdhkarIndex, QIYAM_ADHKAR.length - 1);
  const item = QIYAM_ADHKAR[idx];
  const count = day.qiyamAdhkarRepCounts[item.id] || 0;
  ...
  const advance = useCallback(() => {
    ...
    setTimeout(() => {
      if (idx + 1 >= QIYAM_ADHKAR.length) {
        setDay(d => ({ ...d, qiyamAdhkarCompleted: true }));
        onDone();
      } else {
        setDay(d => ({ ...d, qiyamAdhkarIndex: idx + 1 }));
      }
    }, 380);
  }, [idx, setDay, onDone]);
  ...
  const increment = useCallback(() => {
    ...
    setDay(d => ({ ...d, qiyamAdhkarRepCounts: { ...d.qiyamAdhkarRepCounts, [item.id]: newCount } }));
  ...
  const pct = Math.round((idx / QIYAM_ADHKAR.length) * 100 + (count / item.repetitions) * (100 / QIYAM_ADHKAR.length));
  ...
  return (
    <div style={{ minHeight: "100vh", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <TopBar
          title={`Qiyam · ${idx + 1} / ${QIYAM_ADHKAR.length}`}
          onBack={onExit}
          chip
          right={idx > 0 ? (
