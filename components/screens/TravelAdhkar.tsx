...
import { TRAVEL_ADHKAR, ADHKAR, EVENING_ADHKAR, QIYAM_ADHKAR, IFTAR_ADHKAR, BACKGROUND_SETS, daySetIndexForToday, backgroundFor } from "@/lib/data";
...
const BG_OFFSET = ADHKAR.length + EVENING_ADHKAR.length + QIYAM_ADHKAR.length + IFTAR_ADHKAR.length;
...
export default function TravelAdhkar({
  ...
  const idx = Math.min(day.travelAdhkarIndex, TRAVEL_ADHKAR.length - 1);
  const item = TRAVEL_ADHKAR[idx];
  const count = day.travelAdhkarRepCounts[item.id] || 0;
  ...
  const advance = useCallback(() => {
    ...
    setTimeout(() => {
      if (idx + 1 >= TRAVEL_ADHKAR.length) {
        setDay(d => ({ ...d, travelAdhkarCompleted: true }));
        onDone();
      } else {
        setDay(d => ({ ...d, travelAdhkarIndex: idx + 1 }));
      }
    }, 380);
  }, [idx, setDay, onDone]);
  ...
  const increment = useCallback(() => {
    ...
    setDay(d => ({ ...d, travelAdhkarRepCounts: { ...d.travelAdhkarRepCounts, [item.id]: newCount } }));
  ...
  const pct = Math.round((idx / TRAVEL_ADHKAR.length) * 100 + (count / item.repetitions) * (100 / TRAVEL_ADHKAR.length));
  ...
  return (
    <div style={{ minHeight: "100vh", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        <TopBar
          title={`Voyage · ${idx + 1} / ${TRAVEL_ADHKAR.length}`}
          onBack={onExit}
          chip
          right={idx > 0 ? (
